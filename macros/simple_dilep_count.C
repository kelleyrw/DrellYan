...
void FillHists(rt::TH1Container& hc, TChain& chain, const std::string& sample_name, long num_events = std::numeric_limits<long>::max())
{
    ...
    if (sample_name == "ttotr"  ) {eff_scale = s_SDEfficiency[ 5];}
...
void create_dilep_count(const long num_events = std::numeric_limits<long>::max())
{
    ...
    TChain ch_ttotr("Events");
    ch_ttotr.Add("/hadoop/cms/store/group/snt/papers2012/Summer12_53X_MC/TTJets_HadronicMGDecays_8TeV-madgraph_Summer12_DR53X-PU_S10_START53_V7A_ext-v1/V05-03-24/merged_ntuple_1[0-9].root");
...
void print_dilep_count()
{
    ...
    t1.setTable() (                                                 "mm ",                                   "ee")
                  ( "DY --> mm"   , rt::Integral(hc["h_count_mm_dyll"   ]), rt::Integral(hc["h_count_mm_dyll"   ]))
                  ( "W --> mnu"   , rt::Integral(hc["h_count_mm_wjets"  ]), rt::Integral(hc["h_count_mm_wjets"  ]))
                  ( "tt"          , rt::Integral(hc["h_count_mm_ttdil"  ]), rt::Integral(hc["h_count_mm_ttdil"  ]))
                  ( "QCD"         , rt::Integral(hc["h_count_mm_qcd"    ]), rt::Integral(hc["h_count_mm_qcd"    ]))
                  ( "WW"          , rt::Integral(hc["h_count_mm_ww"     ]), rt::Integral(hc["h_count_mm_ww"     ]))
                  ( "WZ"          , wz_mm                                 , wz_ee                                 )
                  ( "WZ --> 2l2q" , rt::Integral(hc["h_count_mm_wz2l2q" ]), rt::Integral(hc["h_count_mm_wz2l2q" ]))
                  ( "WZ --> 3lnu" , rt::Integral(hc["h_count_mm_wz3l"   ]), rt::Integral(hc["h_count_mm_wz3l"   ]))
                  ( "ZZ"          , zz_mm                                 , zz_ee                                 ) 
                  ( "ZZ --> 2l2q" , rt::Integral(hc["h_count_mm_zz2l2q" ]), rt::Integral(hc["h_count_mm_zz2l2q" ]))
                  ( "ZZ --> 2lnu" , rt::Integral(hc["h_count_mm_zz2l2nu"]), rt::Integral(hc["h_count_mm_zz2l2nu"]))
                  ( "ZZ --> 4l"   , rt::Integral(hc["h_count_mm_zz4l"   ]), rt::Integral(hc["h_count_mm_zz4l"   ]))
                  ;
